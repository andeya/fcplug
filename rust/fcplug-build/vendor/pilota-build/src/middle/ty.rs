...
pub struct Ty {
    pub kind: TyKind,
    pub tags_id: TagId,
    pub(crate) in_stack: Arc<RwLock<Option<bool>>>,
}
...
impl ::core::hash::Hash for Ty {
    fn hash<H: ::core::hash::Hasher>(&self, state: &mut H) {
        ...
        ::core::hash::Hash::hash(&self.tags_id, state);
    }
}
...
impl Display for CodegenTy {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            CodegenTy::Bytes => f.write_str("::std::vec::Vec<u8>"),
        }
    }
}
...
pub(crate) fn fold_ty<F: Folder>(f: &mut F, ty: &Ty) -> Ty {
    let in_stack = Arc::new(RwLock::new(Some(true)));
    let kind = match &ty.kind {
        String => {
            in_stack.write().unwrap().insert(false);
            TyKind::String
        }
        Void => TyKind::Void,
        U8 => TyKind::U8,
        ...
        Vec(ty) => {
            in_stack.write().unwrap().insert(false);
            TyKind::Vec(f.fold_ty(ty).into())
        }
        Set(ty) => {
            in_stack.write().unwrap().insert(false);
            TyKind::Set(f.fold_ty(ty).into())
        }
        Map(k, v) => {
            in_stack.write().unwrap().insert(false);
            TyKind::Map(fold_ty(f, k).into(), fold_ty(f, v).into())
        }
        Path(path) => {
            in_stack.write().unwrap().take();
            TyKind::Path(path.clone())
        }
