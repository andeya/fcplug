...
impl Resolver {
    ...
    fn modify_ty_by_tags(&mut self, mut ty: Ty, tags: &Tags) -> Ty {
        match ty.kind {
            ...
            if tags
                ...
                .unwrap_or(false) =>
                {
                    ty.in_stack.write().unwrap().insert(false);
                    ty.kind = ty::String;
                }
            ty::Bytes
            if tags
            ...
            impl Folder for ArcFolder<'_> {
                fn fold_ty(&mut self, ty: &Ty) -> Ty {
                    ...
                    Ty {
                        ...
                        in_stack: Arc::new(RwLock::new(Some(false))),
                    }
                }
            }
    ...
    fn lower_type(&mut self, ty: &ir::Ty) -> Ty {
        ...
        let kind = match &ty.kind {
            ...
            ir::TyKind::Vec(ty) => {
                in_stack.write().unwrap().insert(false);
                ty::Vec(Arc::from(self.lower_type(ty)))
            }
            ir::TyKind::Set(ty) => {
                in_stack.write().unwrap().insert(false);
                ty::Set(Arc::from(self.lower_type(ty)))
            }
            ir::TyKind::Map(k, v) => {
                in_stack.write().unwrap().insert(false);
            ...
            ir::TyKind::Path(p) => {
                ...
                ty::Path(self.lower_path(p, Namespace::Ty))
            }
    ...
    fn lower_message(&mut self, s: &ir::Message) -> Message {
        Message {
            ...
            all_in_stack: Arc::new(RwLock::new(None)),
        }
    }
