...
pub fn ffi_pb_callee(_args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let new_item = quote! {
        ...
        pub extern "C" fn #new_ident(mut req: ::fcplug::callee::Buffer) -> ::fcplug::callee::FFIResult {
            #raw_item
            ::fcplug::callee::protobuf::callback(::std::stringify!(#new_ident), #raw_ident, &mut req)
        }
    };
    TokenStream::from(new_item)
}
...
/// format description: `#[ffi_fb_callee]`
/// command to check expanded code: `cargo +nightly rustc -- -Zunstable-options
/// --pretty=expanded`
#[proc_macro_attribute]
#[cfg(not(test))] // Work around for rust-lang/rust#62127
pub fn ffi_fb_callee(_args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let new_item = quote! {
        ...
        pub extern "C" fn #new_ident(mut req: ::fcplug::callee::Buffer) -> ::fcplug::callee::FFIResult {
            #raw_item
            let (_resp_, mut _w_) = #raw_ident(::fcplug::callee::flatbuf::FbRequest::try_from_buffer(&mut req)?);
            let _resp_ = #resp_type::create(&mut _w_, &_resp_);
            _w_.finish_minimal(_resp_);
            ::fcplug::callee::FFIResult::ok(::fcplug::callee::ABIResponse::try_into_buffer(_w_).unwrap())
        }
    };
    TokenStream::from(new_item)
}
